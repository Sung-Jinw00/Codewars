...
int main(int ac, char **av)
{
	...
	{
		int *clue = (ac == 2 ? clues_cli : clues[i]), (*exp)[N] = expected[i], **result = SolvePuzzle(clue);
        if (ac == 2)
        {
		...
		if (!result || !compare_solution(result, exp))
		{
			fprintf(stderr, RED "diff !\n" RESET);
			print_answer(result, wrapped_array(exp), clue);
			if (result)
				free_array2(result);
			break;
		}
		...
		{
			print_answer_array(result, wrapped_array(exp), clue);
