...
def decode_morse_irl(bits):
	"""
		...
		The process is as follows:
			1. `decodeBitsAdvanced(bits)` analyzes the lengths of sequences of '1's and '0's
			to determine the timing of dots, dashes, and spaces.
			2. `decodeMorse(morseCode)` converts the decoded dots and dashes into letters
			and words using the `MORSE_CODE` dictionary.
			3. `decode_morse_irl(bits)` combines these steps to directly decode a bit string
		...
		example:
			>>> decode_morse_irl('000000000001111111000000110100011101110...000001111110001111100001000')
			'THE QUICK BROWN FOX JUMPS OVER THE LAZY DOG'
	"""
	return decodeMorse(decodeBitsAdvanced(bits))
