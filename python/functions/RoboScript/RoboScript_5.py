...
class RSUProgram:
	...
	def execute_raw(self, raw_cmd):
		"""
        Execute a sequence of raw RSU commands on a 2D grid.

        how it works:
            - Starts with an initial position and direction on the grid.
            - Interprets each command:
                - 'F': move forward one step, marking the grid with '*'
                - 'L': turn left (90 degrees)
                - 'R': turn right (90 degrees)
            - Expands the grid dynamically as needed to accommodate movement.
            - Returns the final grid representation as a string.

        args:
            raw_commands (list[str]): List of raw commands ('F', 'L', 'R').

        returns:
            str: 2D grid representation as a string, each line separated by '\r\n'.
                 Example:
                 "*  *\r\n * *\r\n  * "
        """
		code = ''.join(raw_cmd)
	...
	def convert_to_raw(self, tokens):
		"""
        Convert a list of RSU tokens into a raw sequence of commands F, L, R.

        how it works:
            - Builds a full command string from tokens.
            - Handles repetitions in parentheses: ( … )n.
            - Eliminates unnecessary repetitions: ( … )0.
            - Ensures pattern definitions are not nested inside parentheses.
            - Replaces pattern calls with their corresponding commands.
            - Expands numeric command repetitions: e.g., F3 -> F F F.

        args:
            tokens (list[str]): List of RSU tokens from get_tokens().

        returns:
            list[str]: List of raw commands 'F', 'L', 'R' ready for execution.

        raises:
            Exception:
                - If pattern definitions are nested in parentheses
                - If undefined patterns are called
                - If resulting commands contain invalid characters
        """
		code = ''.join(tokens)
		i = len(code)
		while i > 0:
	...
	def get_tokens(self):
		"""
        Tokenize the RSU source code into a list of valid tokens.

        how it works:
            - Iterates over each character of the source code.
            - Recognizes valid tokens:
                - Commands: F, L, R (optionally with numeric repeat: F3, L2, etc.)
                - Pattern declarations: pN
                - Pattern calls: PN
                - Parentheses: '(' and ')'
            - Ignores spaces and comments:
                - Single-line comments: // …
                - Multi-line comments: /* … */
            - Performs syntax checks for common errors:
                - Pattern declarations missing identifiers
                - Numbers with leading zeros
                - Invalid characters
                - Standalone numbers or numbers after comments

        returns:
            list[str]: List of tokens in order of appearance.
                       Token types:
                       - 'F', 'L', 'R', 'Fn', 'Ln', 'Rn'
                       - 'pN' (pattern declaration)
                       - 'PN' (pattern call)
                       - '(', ')'

        raises:
            Exception: If any syntax error is detected.
        """
		# print(f"------ source :\n{self.source}\n")
		code = self.source
		print_content("code = ", code)
	...
	def execute(self):
		"""
        Execute the RSU program from source code and return the resulting grid.

        how it works:
            - Tokenizes the source code using `get_tokens()`.
            - Converts tokens to raw commands `using convert_to_raw()`.
            - Executes the raw commands using `execute_raw()`.
